...
//
// V2: minimal rename from V1.  sDist / tDist are renamed to
// sBaseGblDualShift / tBaseGblDualShift, matching the variable name used
// in the general-graph case (edmonds_mwm_csr.cpp).  The Dijkstra-style
// "distance" terminology is replaced with the dual-mechanics terminology
// because the general case operates on edges in the PQ rather than
// vertices, and "distance" has no clean meaning there.  All other
// variable names are unchanged from V1.  Behavior is identical to V1.
...
bool augment(const Graph& graph, int sFirst, Matching& matching) {
    ...
    vector<double> sBaseGblDualShift(graph.sNumVtxs, INF), tBaseGblDualShift(graph.tNumVtxs, INF);
    ...
    // tCrtBaseGblDualShift = sCrtBaseGblDualShift + slack.  If tCrtBaseGblDualShift improves the best known
    ...
    auto relaxFromS = [&](int s) {
        double sCrtBaseGblDualShift = sBaseGblDualShift[s];
        for (int k = graph.off[s]; k < graph.off[s + 1]; ++k) {
            ...
            double tCrtBaseGblDualShift = sCrtBaseGblDualShift + slack;
            if (tCrtBaseGblDualShift < tBaseGblDualShift[t]) {
                tBaseGblDualShift[t] = tCrtBaseGblDualShift;
                tPrev[t] = s;
                if (!tVstd[t]) { tPrcdQue.push_back(t); tVstd[t] = 1; }
                tPrcbQue.push({tCrtBaseGblDualShift, t});                                  // insert, don't update
            }
        }
    };
    ...
    while (!tPrcbQue.empty()) {
        auto [tCrtBaseGblDualShift, t] = tPrcbQue.top(); tPrcbQue.pop();

        // Stale-entry guard: a later push gave t a shorter distance.
        if (tCrtBaseGblDualShift > tBaseGblDualShift[t]) continue;
        ...
        // gblDualShift at least tCrtBaseGblDualShift; keeping all visited sDual
        // values non-negative bounds gblDualShift at allowedGblDualShift
        // from above.  When tCrtBaseGblDualShift >= allowedGblDualShift no legal
        ...
        // need to be adjusted by gblDualShift - tCrtBaseGblDualShift which must be
        // >= 0, thus gblDualShift >= tCrtBaseGblDualShift.  Therefore if
        // tCrtBaseGblDualShift >= allowedGblDualShift we would either make the
        // dual of at least one S-vertex negative or we would not make
        // any difference in terms of weight.
        if (tCrtBaseGblDualShift >= allowedGblDualShift) break;

        if (matching.tMate[t] < 0) {
            // Free T reached.  This is the SAP endpoint.
            tLast = t;
            gblDualShift = tCrtBaseGblDualShift;
            break;
        }
        ...
        sBaseGblDualShift[sNext] = tCrtBaseGblDualShift;
        sPrev[sNext] = t;
        if (!sVstd[sNext]) { sPrcdQue.push_back(sNext); sVstd[sNext] = 1; }

        if (tCrtBaseGblDualShift + matching.sDual[sNext] < allowedGblDualShift) {
            allowedGblDualShift = tCrtBaseGblDualShift + matching.sDual[sNext];
            sLast = sNext;
        }
    ...
    if (tLast < 0) {
        ...
        // tCrtBaseGblDualShift >= allowedGblDualShift fired mid-loop (forward progress structurally
    ...
    for (int s : sPrcdQue) {
        double lclDualShift = gblDualShift - sBaseGblDualShift[s];
        if (lclDualShift > 0) matching.sDual[s] -= lclDualShift;
    }
    for (int t : tPrcdQue) {
        double lclDualShift = gblDualShift - tBaseGblDualShift[t];
...
//     slackness checks and causing the bailout test (tCrtBaseGblDualShift >= allowed)
