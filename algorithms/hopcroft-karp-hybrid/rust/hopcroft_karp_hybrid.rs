...
struct HopcroftKarpHybrid {
    ...
    right_count: usize,
    greedy_size: usize,
    graph: Vec<Vec<usize>>,       /* graph[u] = sorted, deduplicated list of right nodes */
    pair_left: Vec<i32>,
    pair_right: Vec<i32>,
    dist: Vec<i32>,
...
impl HopcroftKarpHybrid {
    fn new(left_count: usize, right_count: usize, edges: &[(usize, usize)]) -> Self {
        let mut graph = vec![Vec::<usize>::new(); left_count];
        for &(u, v) in edges {
    ...
    fn bfs(&mut self) -> bool {
        ...
        self.dist[self.left_count] = i32::MAX;    /* NIL sentinel */
        ...
        while qi < queue.len() {
            ...
            if self.dist[u] < self.dist[self.left_count] {
                /* Index-based iteration: no persistent borrow of self.graph across &mut self */
                let n = self.graph[u].len();
                for i in 0..n {
                    let v = self.graph[u][i];
                    let pn = if self.pair_right[v] == NIL {
                        self.left_count
                    } else {
     ...
     *
     * dist[u] = i32::MAX marks u as a dead end (same as original HK).
     */
    ...
    fn greedy_init_md(&mut self) -> usize {
        ...
        order.sort_unstable_by(|&a, &b| {
            self.graph[a].len().cmp(&self.graph[b].len()).then(a.cmp(&b))
        });
        for u in order {
            if self.pair_left[u] != NIL { continue; }
            let mut best: i32 = -1;
...
fn main() {
    println!("Hopcroft-Karp Hybrid Algorithm - Rust Implementation");
    println!("======================================================\n");
