...
/* ---------- CSR adjacency accessors honoring rvrs ---------- */

static inline size_t srcIdxBegin(const BipartiteGraph& g, const HKPState& s, int32_t u) {
    return s.rvrs ? g.tIdx[u] : g.sIdx[u];
}
static inline size_t srcIdxEnd(const BipartiteGraph& g, const HKPState& s, int32_t u) {
    return s.rvrs ? g.tIdx[u + 1] : g.sIdx[u + 1];
}
static inline const int32_t* srcAdjData(const BipartiteGraph& g, const HKPState& s) {
    return s.rvrs ? g.tAdj.data() : g.sAdj.data();
}
...
int32_t greedyInitMd(const BipartiteGraph& g, BipartiteMatching& m) {
    ...
    for (size_t u = 0; u < g.sNumVtxs; u++) {
        size_t st = g.sIdx[u], en = g.sIdx[u + 1];
        for (size_t j = st; j < en; j++) deg[g.sAdj[j]]++;
    }
...
static int32_t bfsShortestLevel(const BipartiteGraph& g, const BipartiteMatching& m, HKPState& s
    ...
    while (!s.sBfsQueue.empty()) {
        ...
        size_t aBeg = srcIdxBegin(g, s, src);
        size_t aEnd = srcIdxEnd(g, s, src);
        const int32_t* adj = srcAdjData(g, s);
...
static void dfsFindPathsHk(const BipartiteGraph& g, const BipartiteMatching& m, HKPState& s,
    ...
    while (sf != NIL) {
        ...
        while (!s.sDfsStack.empty()) {
            ...
            size_t aBeg = srcIdxBegin(g, s, src);
            size_t aEnd = srcIdxEnd(g, s, src);
            int32_t numEdg = (int32_t)(aEnd - aBeg);
            const int32_t* adj = srcAdjData(g, s) + aBeg;
...
int main(int argc, char* argv[]) {
    printf("Hopcroft-Karp Pure Algorithm - C++ Implementation (CSR)\n");
    printf("=========================================================\n\n");
