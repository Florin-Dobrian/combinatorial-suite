 ...
 *
 * Fields named s_x correspond to the algorithm's source side; t_x to the
 * algorithm's target side. They are NOT the same as the input graph's
 * s_adj/t_adj when rvrs=true.
 */
...
/* ---------- Adjacency / mate accessors honoring rvrs ---------- */

#[inline]
fn src_adj<'a>(g: &'a BipartiteGraph, s: &HKPState, u: i32) -> &'a [i32] {
    if s.rvrs { &g.t_adj[u as usize] } else { &g.s_adj[u as usize] }
}
...
fn dfs_find_paths_hk(
    ...
    while sf != NIL {
        ...
        while !s.s_dfs_stack.is_empty() {
            ...
            let su = src as usize;
            let adj_len = src_adj(g, s, src).len();

            while s.s_edge_idx[su] < adj_len {
                let t = src_adj(g, s, src)[s.s_edge_idx[su]];
...
fn dfs_find_paths_lkhd(
    ...
    while sf != NIL {
        ...
        while !s.s_dfs_stack.is_empty() {
            ...
            while s.s_lkhd_idx[su] < adj_len {
                let t = src_adj(g, s, src)[s.s_lkhd_idx[su]];
                #[cfg(feature = "stats")] { ps.bfs_edg += 1; }
                let tu = t as usize;
                let ss = tgt_mate(m, s, t);
...
fn main() {
    println!("Hopcroft-Karp Pure Algorithm - Rust Implementation (VV)");
    println!("=========================================================\n");
