/*
 * Edmonds' Blossom Algorithm (Simple) - O(V^2 * E) Maximum Matching
 *
 * Basic implementation with BFS augmenting path search and blossom contraction.
...
struct EdmondsBlossomSimple {
    ...
    EdmondsBlossomSimple(int n, const std::vector<std::pair<int,int>>& edges) : n(n) {
        graph.resize(n);
        mate.assign(n, NIL);
        for (auto& e : edges) {
    ...
    int find_base(int v, std::vector<int>& base) {
        while (base[v] != v) v = base[v] = base[base[v]]; /* path compression */
        return v;
    }
    ...
    std::vector<int> find_augmenting_path(int start) {
        ...
        std::vector<int> label(n, 0); /* 0=unlabeled, 1=outer, 2=inner */
            ...
            for (int w : graph[v]) {
                ...
                if (label[w] == 0) {
                    if (mate[w] != NIL) {
                        ...
                        /* found augmenting path — build it */
...
int main(int argc, char* argv[]) {
    ...
    EdmondsBlossomSimple eb(n, edges);
