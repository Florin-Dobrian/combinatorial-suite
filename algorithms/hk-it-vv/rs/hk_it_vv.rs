...
struct HKIState {
    ...
    s_idx: Vec<usize>,         // length s_num_vtxs; position within s's adjacency, persistent within a phase
...
fn build_bipartite_graph(s_num_vtxs: usize, t_num_vtxs: usize,
    ...
    for &(s, t) in edges {
        if s < s_num_vtxs && t < t_num_vtxs {
            s_adj[s].push(t as i32);
            t_adj[t].push(s as i32);
        }
    }
...
fn greedy_init_md(graph: &BipartiteGraph, matching: &mut BipartiteMatching) -> usize {
    ...
    s_order.sort_unstable_by(|&s1, &s2| {
        graph.s_adj[s1].len().cmp(&graph.s_adj[s2].len()).then(s1.cmp(&s2))
    });
 ...
 *
 * state.s_idx[s] is the position within s's adjacency (graph.s_adj[s][state.s_idx[s]]),
...
fn dfs(s_first: usize, graph: &BipartiteGraph, matching: &mut BipartiteMatching, state: &mut HKIState) -> bool {
    ...
    while state.stk_top > 0 {
        ...
        let s_num_edgs = graph.s_adj[s].len();

        let mut pushed = false;
        while state.s_idx[s] < s_num_edgs {
            let t = graph.s_adj[s][state.s_idx[s]];
            let ss = if matching.t_mate[t as usize] == NIL {
                graph.s_num_vtxs
            } else {
                matching.t_mate[t as usize] as usize
    ...
    for s in 0..graph.s_num_vtxs {
        if matching.s_mate[s] != NIL {
            ...
            if t < 0 || (t as usize) >= graph.t_num_vtxs {
                ...
                errors += 1;
            } else if !graph.s_adj[s].binary_search(&t).is_ok() {
...
fn main() {
    println!("Hopcroft-Karp Iterative Algorithm - Rust Implementation (VV)");
    println!("==============================================================\n");
