/*
 * Hopcroft-Karp Bidirectional, Iterative (HKB0) - O(E sqrt(V)) Maximum Bipartite Matching
 *
 * Variant 0 ("G2 style"): two parallel HK-style BFSes, one rooted at free S
 ...
 * / HKB0IState), top-level returns numPhases.
 */
...
static bool bfs(const BipartiteGraph& graph, const BipartiteMatching& matching, HKB0IState& state) {
...
static bool dfsFromS(int32_t sFirst, const BipartiteGraph& graph,
                     BipartiteMatching& matching, HKB0IState& state) {
    state.stkTop = 0;
...
int32_t hkb0IterativeMcm(const BipartiteGraph& graph, BipartiteMatching& matching) {
    HKB0IState state;
...
int main(int argc, char* argv[]) {
    printf("Hopcroft-Karp Bidirectional Iterative (HKB0) - C++ Implementation (CSR)\n");
    printf("=========================================================================\n\n");
    ...
    int32_t numPhases = hkb0IterativeMcm(graph, matching);
