...
struct HopcroftKarpPure {
    s_count: usize,
    t_count: usize,
    /* CSR adjacency: adj_edges[adj_off[s] .. adj_off[s+1]) are the T-side
     * vertices neighboring source s. */
    adj_off: Vec<usize>,
    adj_edges: Vec<usize>,
...
impl HopcroftKarpPure {
    ...
    fn build(&mut self, left_count: usize, right_count: usize,
        ...
        /* Build adjacency in a temporary Vec<Vec> for dedup/sort, then
         * flatten into CSR. */
        let mut tmp: Vec<Vec<usize>> = vec![Vec::new(); self.s_count];
        for &(u, v) in edges {
            if self.rvrs {
                if v < self.s_count && u < self.t_count { tmp[v].push(u); }
            } else {
                if u < self.s_count && v < self.t_count { tmp[u].push(v); }
            }
        }
        for adj in &mut tmp { adj.sort_unstable(); adj.dedup(); }

        self.adj_off = vec![0usize; self.s_count + 1];
        for i in 0..self.s_count { self.adj_off[i + 1] = self.adj_off[i] + tmp[i].len(); }
        self.adj_edges = Vec::with_capacity(self.adj_off[self.s_count]);
        for i in 0..self.s_count { self.adj_edges.extend_from_slice(&tmp[i]); }
    ...
    fn greedy_init(&mut self) -> usize {
        ...
        for s in 0..self.s_count {
            ...
            let as_ = self.adj_off[s];
            let ae = self.adj_off[s + 1];
            for j in as_..ae {
                let t = self.adj_edges[j];
    ...
    fn greedy_init_md(&mut self) -> usize {
        ...
        for s in 0..self.s_count {
            let as_ = self.adj_off[s];
            let ae = self.adj_off[s + 1];
            for j in as_..ae { t_deg[self.adj_edges[j]] += 1; }
        }
    ...
    fn bfs_shortest_level(
        ...
        while !self.s_bfs_queue.is_empty() {
            ...
            let as_ = self.adj_off[su];
            let ae = self.adj_off[su + 1];
            for j in as_..ae {
                let t = self.adj_edges[j];
    ...
    fn dfs_find_paths_hk(
        ...
        while sf != NIL {
            ...
            while !self.s_dfs_stack.is_empty() {
                ...
                let su = s as usize;
                let as_ = self.adj_off[su];
                let num_edg = self.adj_off[su + 1] - as_;

                while self.s_idx[su] < num_edg {
                    let t = self.adj_edges[as_ + self.s_idx[su]];
        ...
        while sf != NIL {
            ...
            while !self.s_dfs_stack.is_empty() {
                ...
                while self.s_lkhd[su] < num_edg {
                    let t = self.adj_edges[as_ + self.s_lkhd[su]];
                    #[cfg(feature = "stats")] { ps.bfs_edg += 1; }
                    let ss = self.t_mate[t];
...
fn validate_matching(
    ...
    matching: &[(usize, usize)], rvrs: bool,
) {
    ...
    for &(u, v) in matching {
        let s_side = if rvrs { v } else { u };
        let target = if rvrs { u } else { v };
        let as_ = adj_off[s_side];
        let ae = adj_off[s_side + 1];
        if !adj_edges[as_..ae].binary_search(&target).is_ok() {
...
fn main() {
    println!("Hopcroft-Karp Pure CSR Algorithm - Rust Implementation");
    println!("====================================================\n");
    ...
    match load_graph(&args[1]) {
        Ok((lc, rc, edges)) => {
            ...
            validate_matching(lc, rc, &hk.adj_off, &hk.adj_edges, &matching, use_rvrs);
