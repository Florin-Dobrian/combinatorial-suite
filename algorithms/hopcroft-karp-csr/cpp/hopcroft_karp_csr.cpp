 ...
 *
 * CSR variant: adjacency stored as (adj_off, adj_edges) flat arrays
...
struct HopcroftKarp {
    ...
    HopcroftKarp(int lc, int rc, const std::vector<std::pair<int,int>>& edges)
        ...
        /* Build adjacency in a temporary vector<vector<int>>, dedup/sort,
         * then flatten into CSR.  This keeps the loading phase identical
         * to the Vec<Vec> version but yields CSR for the hot loops. */
...
/* Validation: check each edge is in the original adjacency via CSR binary search */
...
int main(int argc, char* argv[]) {
    printf("Hopcroft-Karp CSR Algorithm - C++ Implementation\n");
    printf("===================================================\n\n");
