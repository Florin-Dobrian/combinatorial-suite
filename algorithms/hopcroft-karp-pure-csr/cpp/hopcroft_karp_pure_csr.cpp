...
struct HopcroftKarpPure {
    ...
    void build(int left_count, int right_count,
        ...
        /* Build adjacency in a temporary vec-of-vec for dedup/sort, then
         * flatten into CSR. */
        std::vector<std::vector<int>> tmp(s_count);
        for (auto& e : edges) {
        ...
        adj_off.assign(s_count + 1, 0);
        for (int i = 0; i < s_count; i++) adj_off[i + 1] = adj_off[i] + (int)tmp[i].size();
        adj_edges.resize(adj_off[s_count]);
        for (int i = 0; i < s_count; i++)
    ...
    int greedy_init() {
        ...
        for (int s = 0; s < s_count; s++) {
            ...
            int as = adj_off[s], ae = adj_off[s + 1];
            for (int j = as; j < ae; j++) {
                int t = adj_edges[j];
                if (t_mate[t] == NIL) {
    ...
    int greedy_init_md() {
        ...
        for (int s = 0; s < s_count; s++) {
            int as = adj_off[s], ae = adj_off[s + 1];
            for (int j = as; j < ae; j++) t_deg[adj_edges[j]]++;
        }
    ...
    int bfs_shortest_level(
        ...
        while (!s_bfs_queue.empty()) {
            ...
            int num_edg = adj_off[s + 1] - adj_off[s];
            const int* adj = adj_edges.data() + adj_off[s];
...
void validate_matching(int lc, int rc,
    ...
    int errors = 0;
    for (auto& e : matching) {
        int s_side = rvrs ? e.second : e.first;
        int target = rvrs ? e.first  : e.second;
        int as = adj_off[s_side], ae = adj_off[s_side + 1];
        if (!std::binary_search(adj_edges.begin() + as, adj_edges.begin() + ae, target)) {
...
int main(int argc, char* argv[]) {
    printf("Hopcroft-Karp Pure CSR Algorithm - C++ Implementation\n");
    printf("========================================================\n\n");
