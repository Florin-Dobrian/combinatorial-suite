...
*/
  ...
  void G_card_matching::phase_2() {
    ...
    fprintf(stderr, "  phase_2: attempts=%d found=%d\n", attempts, found_count);

    int before = 0;
    forall_nodes(v, G) if (mate[v] != nil) before++;
    before /= 2;
    ...
    int after = 0;
    forall_nodes(v, G) if (mate[v] != nil) after++;
    after /= 2;
    fprintf(stderr, "  matching: %d -> %d (delta=%d)\n", before, after, after - before);
  ...
  */
                      ...
                      bool heur) 
  {
    ...
    while (true) {
      ...
      fprintf(stderr, "[iter %d] size_of_M=%d\n", number_of_iterations, size_of_M);
      fflush(stderr);
      if (heur && (number_of_iterations > 0.5 * heur_factor * (max_size_of_M - size_of_M))) {
        ...
        fprintf(stderr, "  calling phase_1...\n"); fflush(stderr);
        if (phase_1()) { // returns true if there is an augmenting path
          fprintf(stderr, "  phase_1 found SAP, Delta=%d, T.size=%d\n", Delta, T.size()); fflush(stderr);
          phase_2();
        } else {
          fprintf(stderr, "  phase_1: no SAP, done\n"); fflush(stderr);
          break;
        }
      }
    }
